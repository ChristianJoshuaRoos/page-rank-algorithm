...
public class PowerIteratorWithTeleportation{
	...
	public static void main(String[] args) throws FileNotFoundException{
		...
		if (args.length != 1){
			...
			double[][] matrixA = MultiplierWithTeleportation.buildMatrix(map);
			...
			while (thresholdMet == false && counter < 100){
				...
				for (int i = 0; i < matrixA.length; i++){
					for (int j = 0; j < matrixA[i].length; j++){
						tempRankVector[i] += matrixA[i][j] * rankVector[j];					
					}
				}
				...
				for (int i = 0; i < tempRankVector.length; i++){
					sumOfRanks = sumOfRanks + tempRankVector[i];
				}
				
				if (!(0.998 <= sumOfRanks && sumOfRanks >= 1.001)){
					for (int i = 0; i < tempRankVector.length; i++){
						tempRankVector[i] = tempRankVector[i] + ((1 - sumOfRanks) / tempRankVector.length);
					}
				}
